...
def list_unfinished_attempts(
    ...
    print(f"\n=== UNFINISHED ATTEMPTS QUERY ===")
    print(f"Student ID: {current_user.id}")
    print(f"Number of unfinished attempts: {len(attempts)}")
    for attempt in attempts:
        print(f"  Attempt ID: {attempt.id}, End Time: {attempt.end_time}")
    print(f"=== END QUERY ===")
...
def resume_exam(
    ...
    if now > exam.end_time:
        ...
        print(f"Auto-submitting exam attempt {attempt_id} due to time expiry")
...
def submit_exam(
    ...
    print(f"Attempt found: {attempt is not None}")
    ...
    if attempt.end_time is not None:
        print(f"Exam already submitted at {attempt.end_time}")
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        print(f"Score calculation complete. Score: {updated_attempt.score}, Total: {updated_attempt.total_possible_score}")
        print(f"Attempt end_time set to: {updated_attempt.end_time}")
        ...
        print(f"ValueError during score calculation: {e}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        print(f"Unexpected error during score calculation: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
